...
- Rebuilt the example app around two real use cases so it *sells* the library instead of just listing its API: a **Feed** tab (Instagram-style social feed — long-press a photo for a popover peek of the post, tap to open it) and a **Shop** tab (e-commerce store — long-press a product for a bottom-sheet quick-view with price/rating, tap to open the product page), plus an **API** tab (hook / static / headless / scrollview with copy-paste code). The two use cases show the popover and sheet variants in genuinely different real-world contexts. Replaced the earlier abstract Start/Components tabs and the 612-line kitchen-sink `examples.tsx`; added dedicated `PostPreview`/`ProductPreview` peek components and a single `theme.ts`; `detail.tsx` is now a real post/product page (rating stars, price, add-to-cart / buy for products; author + likes + caption for posts).
