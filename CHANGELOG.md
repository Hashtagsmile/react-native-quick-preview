...
- Restructured the example app from 4 overlapping tabs (Overview / API / Basic / Showcase — three of which demoed the same long-press flow) into **3 focused tabs, one concept each**: **Start** (pitch + imperative `useQuickPreview` hero + gallery), **API** (the 3 code entry points), **Components** (the declarative wrappers + an options/lifecycle playground covering `size`, `dismissOn*`, and all four lifecycle callbacks). Deleted the 612-line kitchen-sink `examples.tsx` and the dual-role `addons.tsx`; extracted a shared `PreviewCard`/`PreviewTile` and a single `theme.ts` so the screens stop reimplementing the card five ways; cleaned `detail.tsx` to drive off `item.kind` and dropped its dead rating/reviews/metadata branches.
